...
var getApiFrameworkIndex = function(msg, callback){
   ...
   var index = {
      ...
      apis: [ ],
      info: {
         title: 'PEAT API Framework REST APIs',
         description: '',
...
var transformAPiEndpoint = function(msg, callback){
   ...
   var index = {
      ...
      basePath       : "https" + '://' + msg.headers.host + "/v.04",
      produces : [
         "application/json"
      ],
      consumes : [
      ...
      res.on('end', function() {
         ...
         for ( var i = 0; i < originalIndex.apis.length; i++ ){
            ...
            for ( var j = 0; j < index.apis[i].operations.length; j++){
               ...
               for (var k = 0; k < operation.parameters.length; k++){
                  ...
                  if (index.apis[i].operations[j].parameters[k].name === 'id'  &&
                     index.apis[i].operations[j].parameters[k].dataType  === 'undefined' ){

                     index.apis[i].operations[j].parameters[k].dataType = 'string';
                  }
         ...
         for ( var m in originalIndex.models ){
            if (originalIndex.models.hasOwnProperty(m)) {
               if ("ListView" === m){
                  index.models.CloudletList    = originalIndex.models[m];
                  index.models.CloudletList.id = "CloudletList";

                  index.models["ListView"] = {
                     "properties": {
                        "cbs": {
                           "type"        : "List",
                           "items"       : {
